...
3. Replaces all instances of "notion-gh-pages" with the module name
4. Renames the src/notion_gh_pages directory to src/{modulename}
...
def main():
    ...
    if module_name == "notion_gh_pages":
        print("Module name is already 'notion_gh_pages', no changes needed.")
        return
    ...
    for file_path in files_to_process:
        if find_and_replace_in_file(file_path, "notion_gh_pages", module_name, args.dry_run):
            files_changed += 1
    ...
    if rename_directory("notion_gh_pages", module_name, args.dry_run):
